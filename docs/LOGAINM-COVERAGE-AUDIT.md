...
This audit establishes technical coverage and measurable source completeness; it is not linguistic, onomastic, community, or political approval. The [independent Northern Ireland source and hierarchy review](LOGAINM-NORTHERN-IRELAND-REVIEW.md) inspects the 229 countyless and 312 multi-county records; its specialist, licensing, and upstream-data actions remain open. Re-run both audits after every monthly ingest and block artifact promotion when `releaseGate` is `fail`.
