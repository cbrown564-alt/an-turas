...
| Date | Work | Where |
|---|---|---|
| 2026-08-01 | **D29 freeze-run shell follow-up.** The first Clew Bay task now keeps its authored Irish/English context visible inside the shared one-screen shell; matching clears the mismatch cue on the repairing tap; exercise-to-exercise transitions avoid a transient accessibility frame and each task resets its scroll position. Focused build and 24 unit/contract tests pass; the first task and direct construction path were inspected on iPhone 17 Pro. The full nine-step UI walk remains blocked on iOS 26.5 because XCUI reports construction token `as` as not hittable only when arriving from matching; direct construction launch passes. | `ios/AnTuras/CountyExerciseSystem.swift`, `ios/AnTuras/CountyStoryExperienceView.swift`, `ios/AnTurasUITests/FreezeRunUITests.swift` |
