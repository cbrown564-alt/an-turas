...
struct AssembleView: View {
    ...
    init(block: AssembleBlock, isLast: Bool, onContinue: @escaping () -> Void, onSolved: @escaping () -> Void) {
        ...
        _bank = State(initialValue: block.tiles.shuffled())
    }
            ...
            ExerciseCard(prompt: block.prompt) {
                VStack(alignment: .leading, spacing: 10) {
                    FlowLayout(spacing: 8) {
                        ForEach(chosen, id: \.self) { word in
                            Tile(word: word) {
                                guard !solved else { return }
                                chosen.removeAll { $0 == word }
                                bank.append(word)
                            }
                        }
                    }
    ...
    private func check() {
        guard !solved else { return }
        if chosen.joined(separator: " ") == block.answer {
            solved = true
                    ...
                    if block.fada {
                        HStack(spacing: 6) {
                            ForEach(["á", "é", "í", "ó", "ú"], id: \.self) { fada in
                                Button(fada) { if !solved { text.append(fada) } }
                                    .font(.system(size: 16, design: .serif))
                                    .foregroundStyle(Theme.ink)
                                    .padding(.vertical, 4)
    ...
    private func pairButton(_ item: String, isLeft: Bool) -> some View {
        Button {
            ...
            Text(item)
                ...
                .clipShape(RoundedRectangle(cornerRadius: 3))
                .overlay(RoundedRectangle(cornerRadius: 3).stroke(border(item), lineWidth: 1))
        }
        .buttonStyle(.plain)
